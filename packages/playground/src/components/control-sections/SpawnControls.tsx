...
import { Particle } from "@cazala/party/legacy";
  ...
  (
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        setState: (state) => {
          if (state.defaultSize !== undefined)
            setParticleSize(state.defaultSize);
          if (state.defaultMass !== undefined)
            setParticleMass(state.defaultMass);
          ...
          if (state.drawStepSize !== undefined)
            setDrawStepSize(state.drawStepSize);
          if (state.pinned !== undefined) setIsPinned(state.pinned);
          if (state.shapeSides !== undefined) setShapeSides(state.shapeSides);
          if (state.shapeLength !== undefined)
            setShapeLength(state.shapeLength);
        },
      }),
      [
        particleSize,
