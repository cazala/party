...
export function useSpawner({ getSystem, getRenderer, getCanvas }: UseSpawnerProps) {
  ...
  // Streaming functions
  const startStreaming = useCallback((x: number, y: number, size: number) => {
  ...
  const updateVelocityPreview = useCallback(() => {
    ...
    const velocity = calculateVelocity(mouseState.startPos, mouseState.currentPos);
  ...
  const updateSizePreview = useCallback(() => {
    ...
    const size = calculateParticleSize(distance, mouseState.isDragging, mouseState.dragThreshold);
    const previewParticle = createParticle(
      mouseState.startPos.x,
      mouseState.startPos.y,
      size,
  ...
  const onMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (!mouseState.shiftPressed) {
      if (!wasCmdPressed && mouseState.cmdPressed) {
        ...
        const distance = getDistance(mouseState.startPos, mouseState.currentPos);
        mouseState.velocityModeSize = calculateParticleSize(distance, mouseState.isDragging, mouseState.dragThreshold);
        updateVelocityPreview();
        return;
  ...
  }, [getCanvas, getRenderer, stopStreaming, startStreaming, updateVelocityPreview, updateSizePreview]);
