import { System, Canvas2DRenderer } from "@cazala/party/legacy";
...
export function calculateCameraToShowWorldBounds(
  ...
  const effectiveWorldWidth = Math.max(
    savedWorldBounds.worldWidth,
    minWorldSize
  );
  const effectiveWorldHeight = Math.max(
    savedWorldBounds.worldHeight,
    minWorldSize
  );
