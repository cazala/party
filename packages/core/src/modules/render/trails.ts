...
export class Trails extends Module<"trails", TrailsInputs> {
  ...
  webgl2(): WebGL2Descriptor<TrailsInputs> {
    // WebGL2 Trails module uses two compute-like passes over the scene texture:
    // 1. Decay pass: exponential fade toward clear color with alpha decay
    // 2. Diffuse pass: gaussian-like blur with configurable radius
    // Both read from input texture and write to output, participating in ping-pong.
    return {
      passes: [
        // Decay pass: fade existing content toward background color
        {
          kind: RenderPassKind.Compute,
          kernel: ({ getUniform, readScene, writeScene }) => `{
  let coords = vec2<i32>(i32(gid.x), i32(gid.y));
  ...
  if (all(abs(out_rgb - bg) < vec3<f32>(eps)) && out_a < eps) {
    ...
    ${writeScene("coords", "vec4<f32>(out_rgb, out_a)")};
        ...
        // Diffuse pass: gaussian-like blur
        {
          kind: RenderPassKind.Compute,
          kernel: ({ getUniform, readScene, writeScene }) => `{
  let coords = vec2<i32>(i32(gid.x), i32(gid.y));
  let dims = textureDimensions(input_texture);
  let radius_i = clamp(i32(round(${getUniform("trailDiffuse")})), 0, 12);
  if (radius_i <= 0) { ${writeScene("coords", `${readScene("coords")}`)}; return; }
  let sigma = max(0.5, f32(radius_i) * 0.5);
  let twoSigma2 = 2.0 * sigma * sigma;
  var sum = vec4<f32>(0.0, 0.0, 0.0, 0.0);
  var wsum: f32 = 0.0;
