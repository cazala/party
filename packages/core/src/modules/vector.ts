export class Vector {
   ...
   */
  static getPooled(x: number = 0, y: number = 0): Vector {
    let vector: Vector;

    if (Vector.pool.length > 0) {
      vector = Vector.pool.pop()!;
      vector.set(x, y);
    } else {
   ...
   */
  returnToPool(): void {
    if (this.isPooled && Vector.pool.length < Vector.maxPoolSize) {
      this.isPooled = false;
      this.x = 0;
      this.y = 0;
  ...
  static setMaxPoolSize(maxSize: number): void {
    Vector.maxPoolSize = Math.max(0, maxSize);

    // Trim pool if it exceeds new max size
    if (Vector.pool.length > Vector.maxPoolSize) {
      Vector.pool.length = Vector.maxPoolSize;
    }
  }
  ...
  static getPoolStats(): { poolSize: number; maxPoolSize: number } {
    return {
      poolSize: Vector.pool.length,
      maxPoolSize: Vector.maxPoolSize,
    };
  }
