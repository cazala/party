 ...
 */
export function generateForceFragmentShader(
  forceFunctionDeclarations: string,
  forceCallCode: string,
...
// Module-specific uniforms (generated by WebGL2Engine)
${uniformDeclarations}
...
float GRID_MAXY() {
  // Bugfix: must mirror GRID_MINY (divide by 2*zoom), not 2/zoom.
  float halfH = u_viewSize.y / (2.0 * max(u_viewZoom, 0.0001));
  return u_viewOffset.y + halfH;
}
...
// Per-module apply functions (generated by WebGL2Engine).
// IMPORTANT: Modules often use 'return;' for early exit; these must return from the
// per-module function, not from 'main()', otherwise we'd skip writing 'fragColor'.
${forceFunctionDeclarations}
...
void main() {
  ...
  // Apply forces (generated call-site)
${forceCallCode}

  // Write back modified data
  if (texelOffset == 0) {
    // Position and velocity may be modified by some force modules (e.g. boundary).
    fragColor = vec4(p.position, p.velocity);
  } else if (texelOffset == 1) {
    // Acceleration is updated by forces; size/mass may also be modified (e.g. boundary kill).
    fragColor = vec4(p.acceleration, p.size, p.mass);
  } else {
    // Color unchanged (but keep hook for modules that may override color later).
    fragColor = p.color;
  }
}
`;
...
/**
 * Constraint pass: run post-integration position constraints (e.g. collisions).
 * Similar to force pass but intended to mutate position/velocity/mass deterministically.
 *
 * IMPORTANT: We provide a simple brute-force getNeighbors() implementation so
 * neighbor-based constraints can work even though WebGL2 grid sorting is not complete yet.
 */
export function generateConstrainFragmentShader(
  constrainFunctionDeclarations: string,
  constrainCallCode: string,
  uniformDeclarations: string = ""
): string {
  return `#version 300 es
precision highp float;
...
// View uniforms (for GRID_* helpers used by some constraints)
uniform vec2 u_viewOffset;
uniform float u_viewZoom;
uniform vec2 u_viewSize;

// Global max particle size (used by some neighbor-based constraints)
...
// Brute-force neighbor query (fallback until WebGL2 grid sorting is implemented).
// Signature matches neighborIteratorHelpers in this file.
int getNeighbors(vec2 pos, float radius, int selfIndex, out int neighbors[64], int maxCount) {
  int count = 0;
  float r2 = radius * radius;
  // Hard cap to keep shader loops bounded. This keeps collisions functional for
  // small/medium particle counts (like the playground defaults).
  const int MAX_SCAN = 4096;
  int limit = min(u_particleCount, MAX_SCAN);
  ...
  for (int j = 0; j < MAX_SCAN; j++) {
    ...
    vec4 other0 = texture(u_particleTexture, otherCoord0);
    vec4 other1 = texture(u_particleTexture, otherCoord1);

    vec2 otherPos = other0.xy;
    float otherMass = other1.w;
...
// Per-module constraint functions (generated by WebGL2Engine).
// IMPORTANT: Modules often use 'return;' for early exit; these must return from the
// per-module function, not from 'main()', otherwise we'd skip writing 'fragColor'.
${constrainFunctionDeclarations}
...
void main() {
  ...
  vec4 texel2 = texture(u_particleTexture, coord2);
...
${constrainCallCode}
  ...
  // Write back (same packing as other passes)
  if (texelOffset == 0) {
    fragColor = vec4(p.position, p.velocity);
  } else if (texelOffset == 1) {
    fragColor = vec4(p.acceleration, p.size, p.mass);
